...
def gd(g,o):
    ...
    for r in g.get_organization(o).get_repos():
        ...
        x={"name":r.name,"full_name":r.full_name,"description":r.description or"—","language":r.language or"—","created_at":r.created_at.strftime("%d-%m-%Y"),"updated_at":r.updated_at.strftime("%d-%m-%Y"),"stars":r.stargazers_count,"forks":r.forks_count,"size_mb":round(r.size/1024,1) if r.size else 0}
...
def gt(d):
    ...
    h=["Проект Project","Описание Description","Язык Language","Создан Created","Обновлён Updated","⭐","🍴","Размер (МБ) Size (MB)","README","Релизов Releases","Скачиваний Downloads"]
    b="| "+" | ".join(h)+" |\n| "+" | ".join([":---:"]*len(h))+" |\n"
    for r in d:
        p=r['description'][:40]+"…" if len(r['description'])>40 else r['description']
        b+="| "+" | ".join([f"[{r['name']}](https://github.com/{r['full_name']})",p,r['language'],r['created_at'],r['updated_at'],str(r['stars']),str(r['forks']),str(r['size_mb']),"✅" if r['has_readme'] else"❌",str(r['release_count']),str(r['total_downloads'])])+" |\n"
    return b
def u(p,t):
